...
async def test_crud(cwm_test_db):
    ...
    secret_key = created_bucket.pop('secret_key')
    ...
    assert bucket.keys() == {'access_key', 'blocked', 'bucket_name', 'instance_id', 'public'}
    ...
    assert bucket['access_key'].startswith(f'{bucket_name}:')
    assert len(bucket['access_key']) == len(bucket_name) + 11
